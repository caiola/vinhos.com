...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("ad", schema=None) as batch_op:
        batch_op.drop_constraint("ad_unique_account_id_and_title", type_="unique")
        batch_op.create_index("ad_unique_id_title", ["id", "title"], unique=False)

    with op.batch_alter_table("account", schema=None) as batch_op:
        batch_op.add_column(sa.Column("name", mysql.VARCHAR(length=50), nullable=False))
        batch_op.add_column(sa.Column("nif", mysql.VARCHAR(length=50), nullable=False))
