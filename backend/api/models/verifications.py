...
class Verification(db.Model, BaseModel, metaclass=MetaBaseModel):
    ...
    action = db.Column(db.String(10), nullable=False, comment="Verification action e.g. user")
    token = db.Column(db.String(64), nullable=False, unique=True, comment="Unique token")
    date_created = db.Column(db.DateTime(timezone=True), default=func.utcnow(), comment="Date when token was created")
